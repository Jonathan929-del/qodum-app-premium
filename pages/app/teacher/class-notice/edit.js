...
export default function App({navigation, route}) {
    ...
    const submitHandler = async () => {
        ...
        try {
            ...
            const params = {
                class_notice_id:Number(notice.class_notice_id),
                title,
                body:message
            };
            const notificationLink = `${configs.EXPO_PUBLIC_API_URL}/notifications/edit-class-notice`;
            await axios.post(notificationLink, params);

            // Reseting
            setMessage('');
            setStates({...states, loading:false});
            navigation.navigate('teacher-class-notice', {isEdited:true});
        }catch(err){
            console.log(err);
        }
    };
